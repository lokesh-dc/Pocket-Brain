...
export const CATEGORY_CONFIG: Record<string, { icon: any; color: string; accent: string }> = {
  ...
  media: { icon: Clapperboard, color: '#8b5cf6', accent: '#8b5cf6' }, // Violet
...
export const getCategoryConfig = (name: string = '') => {
  ...
  if (normalized.includes('media') || normalized.includes('movie') || normalized.includes('song')) return CATEGORY_CONFIG.media;
